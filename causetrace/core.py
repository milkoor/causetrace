...
def _event_fingerprint(ev: ToolEvent) -> str:
    payload = {
        ...
        "tool_input": _safe_serialize(ev.tool_input),
        "tool_output": _safe_serialize(ev.tool_output),
    }
    raw = _stable_serialize(payload).encode("utf-8", "replace")
...
def _remap_parent_event_id(parent_event_id: Optional[str], id_map: Dict[str, str]) -> Optional[str]:
    if not parent_event_id:
        return parent_event_id
    parents = [p.strip() for p in parent_event_id.split(",") if p.strip()]
    if not parents:
        return parent_event_id
    return ",".join(id_map.get(parent, parent) for parent in parents)
...
class JSONStore:
    ...
    def append_missing(
        ...
        *,
        ...
        """Append only events whose stable content fingerprint is not stored.

        Event IDs generated by post-hoc parsers are not stable. Deduplication
        therefore ignores event_id and parent_event_id, then remaps parents of
        newly appended events to the matching stored IDs when possible.
        """
        existing_events = self.load(session_id)
        fingerprint_to_id = {
            _event_fingerprint(event): event.event_id
        ...
        for event in events:
            ...
            if existing_id:
                id_map[event.event_id] = existing_id
                skipped += 1
                continue

            remapped_parent = _remap_parent_event_id(event.parent_event_id, id_map)
            if not dry_run:
                event.parent_event_id = remapped_parent
                self.append(session_id, event)

            fingerprint_to_id[fingerprint] = event.event_id
            id_map[event.event_id] = event.event_id
            added += 1
